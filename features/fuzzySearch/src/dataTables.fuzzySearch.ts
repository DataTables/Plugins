		...
		if (
			...
			// Otherwise perform a fuzzy search
				...
				api.rows().iterator(
					'row',
					function (settings: any, rowIdx) {
						settings.data[rowIdx]._fuzzySearch = fuzzySearch(
							fuzzySearchVal,
							settings.data[rowIdx]?.searchCellCache,
							initialFuzzy
						);
					},
					false
...
function fuzzySearch(searchVal: any, data: any, initial: any) {
	var x, y, i;
